...
import copy
import pickle
import Missile_Gen3_ENV_3D as MissileEnv
...
actor_weightfile = 'params/anet_params_R,initL,Ohm'+str(sys.argv[2])+'.pkl'
...
Transition = namedtuple('Transition', ['s', 'a', 'r', 's_', 'Md'])
...
with torch.no_grad():
    for i_ep in range(5000000):
        ...
        while t<max_step_count:
            ...
            action = ac_model(state)
            Acc_cmd = Vector3(0, action[0], action[1])

            MissileModel_1.simulate(Acc_cmd)
            
            Rmag, Look, LOSdiot, _, state_ = MissileSeeker_1.seek(t)
            #print(state_)
            ...
            # agent.store_transition(Transition(state, np.array([Acc_cmd.y,Acc_cmd.z]), score, state_, proceeding))
            ...
            TrajData.append(MissileModel_1.pos.vec.tolist())
        ...
        if M_is_hit:
            ...
            #env.close()
            saveCount += 1
            TrajData = np.array(TrajData)
            fig = plt.figure(figsize=(10, 10))
            ax = fig.add_subplot(111, projection='3d')
            ax.scatter(TrajData[:,0], TrajData[:,1], TrajData[:,2], s=1, c='black')
            ax.scatter(0, 0, 0, c='red', marker='^')
            ax.set_xlabel('X Label')
            ax.set_ylabel('Y Label')
            ax.set_zlabel('Z Label')
            fig.savefig("./img/traj"+str(SessionIdentificationNumber)+str(i_ep)+".png")
